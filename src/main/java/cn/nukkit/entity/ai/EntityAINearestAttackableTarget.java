...
public class EntityAINearestAttackableTarget<T extends EntityLiving> extends EntityAITarget{
            ...
            public boolean apply(T p_apply_1_){
                if (targetSelector != null && !targetSelector.apply(p_apply_1_)){
                    ...
                    if (p_apply_1_ instanceof Player){
                        ...
                        if ((double)p_apply_1_.distance(EntityAINearestAttackableTarget.this.taskOwner) > d0){
                            return false;
                        }
                    }
    ...
    public boolean shouldExecute(){
        if (this.targetChance > 0 && this.taskOwner.getRNG().nextInt(this.targetChance) != 0){
            ...
            List<T> list = this.taskOwner.level.<T>getEntitiesWithinAABB(this.targetClass, this.taskOwner.getBoundingBox().clone().expand(d0, 4.0D, d0), Predicates.<T>and(this.targetEntitySelector, EntitySelectors.NOT_SPECTATING));
    ...
    public static class Sorter implements Comparator<Entity>{
            ...
            public int compare(Entity p_compare_1_, Entity p_compare_2_){
                double d0 = this.theEntity.distanceSq(p_compare_1_);
                double d1 = this.theEntity.distanceSq(p_compare_2_);
